...
from evo_cli.commands.opencode import get_global_config_path, load_jsonc, save_jsonc
...
PLANNOTATOR_OPENCODE_PLUGIN = "@plannotator/opencode@latest"
...
def opencode_config_path(scope):
    if scope == "project":
        return Path.cwd() / "opencode.json"
    return get_global_config_path()


def ensure_opencode_plugin(scope):
    global_path = get_global_config_path()
    if not shutil.which("opencode") and not global_path.exists():
        info("OpenCode is not on PATH; skipping its plugin. Run `evo setup opencode` first.")
        return True

    path = opencode_config_path(scope)
    try:
        existing = load_jsonc(path) if path.exists() else {}
    except ValueError as exc:
        error(f"Could not parse {path}: {exc}")
        return False

    plugins = existing.setdefault("plugin", [])
    if any(str(entry).startswith("@plannotator/opencode") for entry in plugins):
        info("OpenCode plugin [accent]@plannotator/opencode[/accent] already configured")
        return True

    plugins.append(PLANNOTATOR_OPENCODE_PLUGIN)
    header = (
        "// OpenCode project configuration generated by evo"
        if scope == "project"
        else "// OpenCode global configuration generated by evo"
    )
    save_jsonc(path, existing, header=header)
    success(f"Added [accent]{PLANNOTATOR_OPENCODE_PLUGIN}[/accent] to {path}")
    return True
...
def install_plannotator(options):
    ...
    if options["minimal"]:
        ...
        plugin_ok = ensure_opencode_plugin(options["scope"]) and plugin_ok
...
def print_summary(results, scope):
    ...
    if "plannotator" in usable:
        ...
        lines.append("OpenCode picks it up via [accent]@plannotator/opencode[/accent] in its config.")
    ...
    if lines:
        lines.append("")
        lines.append("Restart Claude Code and OpenCode so the new plugins, hooks and skills load.")
...
def agent_toy(names, list_only, scope, minimal):
    ...
    and the plannotator Claude plugin need the Claude Code CLI (`evo setup claude`);
    the plannotator OpenCode plugin is written into opencode.json(c) when OpenCode
    is detected; agent-skills needs Node.js 18+ for npx.
    """
    if list_only:
        print_catalogue()
        return
